...
def make_wts_and_bias(weights, biases, input_channels, output_channels, type):
    ...
    if type == 'normal':
        ...
        weights[new_w] = tf.get_variable('W' + str(curr_weights_num + 1), shape=(3, 3, input_channels, output_channels), initializer= tf.random_normal_initializer(0, 0.02))
...
def conv_net(x, z):
    ...
    i = tf.add(x, z)
    conv1 = conv_block(i, weights, biases, bns, 32, 'leaky_relu', 1)
    ...
    conv5 = conv_block(conv4, weights, biases, bns, 3, 'tanh', 0)
    out = tf.add(conv5, x)
